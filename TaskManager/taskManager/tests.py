...
class TaskAndUsersTest(TestCase):
    ...
    def test_user_form_invalid(self):
        user_form = UserProfile(data={"username": "tester", "password1": "1234567890", "password2": "1234567890"})
        self.assertTrue(not user_form.is_valid())

    def test_user_form_valid(self):
        user_form = UserProfile(data={"username": "tester_two", "password1": "userpassword", "password2": "userpassword"})
        self.assertTrue(user_form.is_valid())
        self.assertTrue(isinstance(user_form.save(commit=False), User))

    def test_task_form_invalid(self):
        task_form = TaskAdding(data={
            'title': "test task",
            'finish': datetime(2021, 6, 10, 8, 0, tzinfo=timezone.utc),
            'priority': '1',
            'status': '2',
...
class ViewsTest(TestCase):
    ...
    def test_add_task_post_valid(self):
        ...
        self.assertEqual(Task.objects.get(title='task tester add post').title, 'task tester add post')
        self.assertEqual(response.status_code, 302)
    ...
    def test_profile(self):
        user = self.user
        self.assertTrue(self.client.login(username=user.username, password="12345"))
        self.assertTrue(self.client.post(reverse('add_task'), {
            'title': 'task 1',
            'finish': datetime(2021, 6, 10, 8, 0, tzinfo=timezone.utc),
            'priority': '2',
            'status': '0',
    ...
    def test_change_password_post_valid(self):
        ...
        self.assertTrue(self.client.login(username=user.username, password="pol1234567890"))
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, '/accounts/profile/')
    ...
    def test_task_edit_valid(self):
        ...
        self.assertTrue(self.client.login(username=user.username, password="12345"))
        task = self.first_task
        response = self.client.post(fr"/edit/{task.id}", {
            'title': "first task",
            'finish': timezone.now(),
            'priority': '1',
            'status': '1',
